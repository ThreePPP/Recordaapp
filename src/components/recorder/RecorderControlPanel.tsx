...
// ── Sub-components (split to avoid re-rendering unrelated sections) ────────────

/** Audio toggle button (System Audio / Mic) */
const AudioToggleBtn = memo(function AudioToggleBtn({
  ...
  return (
    <button type="button" disabled={disabled} onClick={onClick} aria-pressed={active}
      className={`audio-btn flex-col h-auto py-3 ${active ? "active" : ""}`}>
      {icon}
      <span className="mt-1 text-[0.65rem] leading-none">{label}</span>
      <span className={`mt-1.5 text-[0.55rem] font-bold px-1.5 py-0.5 rounded ${
        active ? "bg-cyan-500/15 text-cyan-400" : "text-slate-600"
      }`}>{active ? "ON" : "OFF"}</span>
    </button>
  );
});
...
export const RecorderControlPanel = memo(function RecorderControlPanel({
  ...
  const handleMicToggle = useCallback(async () => {
    if (includeMicrophone) { onMicrophoneChange(false); return; }
    const granted = await window.electronAPI?.requestMicrophonePermission?.();
    if (granted !== false) onMicrophoneChange(true);
        ...
        <div className="mt-auto grid gap-2 border-t border-slate-800/50 pt-4">
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => void onStartSharing()}
              className={`${BASE_BTN} bg-cyan-600 text-white hover:bg-cyan-500 shadow-[0_0_12px_rgba(34,211,238,0.2)]`}>
              <IconMonitor /> {t.preview}
            </button>
            <button type="button" onClick={onStopSharing} disabled={!isSharing}
              className={`${BASE_BTN} border border-slate-700 bg-transparent text-slate-300 hover:bg-slate-800`}>
              {t.stop}
            </button>
          </div>

          {!isRecording ? (
            <button type="button" onClick={() => void onStartRecording()}
              className={`${BASE_BTN} bg-rose-600 text-white hover:bg-rose-500 shadow-[0_0_14px_rgba(244,63,94,0.25)] hover:shadow-[0_0_22px_rgba(244,63,94,0.4)]`}>
              ● {t.startRecording}
            </button>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={isPaused ? onResumeRecording : onPauseRecording}
                className={`${BASE_BTN} border border-amber-900/50 bg-amber-950/30 text-amber-500 hover:bg-amber-900/50`}>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={onClearCrop} disabled={!hasCrop}
              className={`${BASE_BTN} border border-amber-900/30 bg-amber-950/20 text-amber-500/70 hover:bg-amber-900/50`}>
              {t.clearCrop}
            </button>
            {!isRecording && latestRecordingPath ? (
              <button type="button" onClick={() => void window.electronAPI?.openPath?.(latestRecordingPath)}
                className={`${BASE_BTN} border border-emerald-900/30 bg-emerald-950/20 text-emerald-500/80 hover:bg-emerald-900/50`}>
                📂 {t.openLatest}
              </button>
            ) : <div />}
          </div>
