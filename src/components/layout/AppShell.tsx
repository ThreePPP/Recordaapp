...
export const AppShell = memo(function AppShell({ children }: AppShellProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-[1600px] flex-col gap-4 px-3 py-5 sm:px-5 sm:py-7 md:px-8 md:py-9">
      ...
      {/* ── Update Banner ───────────────────────────────────────── */}
      {showBanner && (
        <div className={`flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-2.5 text-sm font-medium ${
          ...
          <span className="flex items-center gap-2">
            {update.status === "downloaded" ? "✓" : "↓"}
            {update.status === "downloaded" ? t.updateReady : `${t.updateAvailable} v${update.version}…`}
          </span>
